...
public class JLog {
    ...
    public static void setGlobalTag(@Nullable String tag) {
        mGlobalTag = tag;
    }
    public static void printStackTrace() {
        ...
        Log.w(mGlobalTag, "║   " + "current thread name : " + Thread.currentThread().getName());
        //ignore top 3 traces.
        for(int i=3;i<length;i++) {
            ...
            Log.w(mGlobalTag,headString);
        }
        printStackTraceLine(mGlobalTag, false);

    }

    public static void printAnrStackTrace(String name, StackTraceElement[] traces) {
        int length = traces.length;
        printLine("ANR_" + name + "_thread", mGlobalTag, true);

        for(int i=0;i<length;i++) {
            StackTraceElement targetElement = traces[i];
            String className = targetElement.getClassName();
            String[] classNameInfo = className.split("\\.");
            if (classNameInfo.length > 0) {
                className = classNameInfo[classNameInfo.length - 1] + SUFFIX;
        ...
        printLine("ANR_"+name+"_thread", mGlobalTag, false);
    }
...
//        String tag = (tagStr == null ? TAG_DEFAULT : tagStr);
    ...
    private static void printStackTraceLine(String tag, boolean isTop) {
        if (isTop) {
            Log.w(tag, "╔════════════════════════════════ Stack Trace Begin ══════════════════════════════════");
        } else {
            Log.w(tag, "╚════════════════════════════════ Stack Trace End ════════════════════════════════════");
        }
    }

    private static void printLine(String type, String tag, boolean isTop) {
        if (isTop) {
            Log.w(tag, "╔════════════════════════════════" + type +  " Begin" + "══════════════════════════════════");
        } else {
            Log.w(tag, "╚════════════════════════════════" + type +  " End" + "════════════════════════════════════");
        }
    }
